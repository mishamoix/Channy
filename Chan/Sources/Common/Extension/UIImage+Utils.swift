...
//  Chan
//
//  Created by Mikhail Malyshev on 10.09.2018.
...
extension UIImage {
    ...
    func resizeForScreen(newSize: CGSize) -> UIImage {
        UIGraphicsBeginImageContextWithOptions(newSize, true, 0)
        
        UIColor.white.setFill()
        
        let rectangle = UIBezierPath(rect: CGRect(x: 0, y: 0, width: newSize.width, height: newSize.height))
        rectangle.fill()
    ...
    func resizeForScreenTransparent(newSize: CGSize) -> UIImage {
        UIGraphicsBeginImageContextWithOptions(newSize, true, 0)
    ...
    func applyBlurEffect(radius: CGFloat) -> UIImage {
        var blurRadius: CGFloat = radius
        let imageToBlur = CIImage(image: self)
        let blurFilter = CIFilter(name: "CIGaussianBlur")
        blurFilter?.setValue(NSNumber(value: Float(blurRadius)), forKey: "inputRadius")
        blurFilter?.setValue(imageToBlur, forKey: "inputImage")
        var resultImage = blurFilter?.value(forKey: "outputImage") as! CIImage
        blurRadius = 0.0
        resultImage = resultImage.cropped(to: CGRect(x: blurRadius, y: blurRadius, width: (imageToBlur?.extent.size.width)! - blurRadius * 2, height: (imageToBlur?.extent.size.height)! - blurRadius * 2))
        let context = CIContext()
        let filteredCGImageRef = context.createCGImage(
            resultImage,
            from: resultImage.extent)
        
        let blurredImage = UIImage(cgImage: filteredCGImageRef!)
        return blurredImage
    }
    
    func borderOfImage(borderWidth: Float) -> UIImage {
        let mainRect = CGRect(x: 0.0, y: 0.0, width: self.size.width, height: self.size.height)
        ...
        shapeLayerPath.usesEvenOddFillRule = true
        ...
        let context = UIGraphicsGetCurrentContext()
        context!.saveGState()
        shapeLayerPath.addClip()
        self.draw(in: mainRect)
    ...
    func applyBlur(radius blurRadius: CGFloat, tintColor: UIColor? = nil, saturation: CGFloat = 1.5, maskImage: UIImage? = nil) -> UIImage? {
        ...
        let hasSaturationChange = fabs(saturation - 1.0) > .ulpOfOne
        ...
        if hasBlur || hasSaturationChange {
            ...
            let effectInContextOptional = UIGraphicsGetCurrentContext()
            
            guard let effectInContext = effectInContextOptional else {
                return nil
            }
            ...
            UIGraphicsBeginImageContextWithOptions(size, false, screenScale)
            let effectOutContextOptional = UIGraphicsGetCurrentContext()
            
            guard let effectOutContext = effectOutContextOptional else {
                return nil
            }
            ...
            if hasBlur {
                ...
                let radiusCalculated = inputRadius * 3.0 * CGFloat(sqrt(2 * Double.pi)) / 4
                var radius = UInt32( floor(radiusCalculated + 0.5) )
    ...
    func testBlurImage(radius: CGFloat) -> UIImage {
