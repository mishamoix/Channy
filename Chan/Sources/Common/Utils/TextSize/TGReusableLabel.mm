        ...
        for (id bridgedLine in _textLines)
        {
            ...
            if (intersectionRange.length != 0)
            {
                ...
                /*if (resultHadRTL)
                {
                    ...
                    startX = floor(startX + lineOrigin.x);
                    endX = ceil(endX + lineOrigin.x);
                }
                else*/
                {
                    startX = ceil(CTLineGetOffsetForStringIndex(line, intersectionRange.location, NULL) + lineOrigin.x);
                    endX = ceil(CTLineGetOffsetForStringIndex(line, intersectionRange.location + intersectionRange.length, NULL) + lineOrigin.x);
                }
                ...
                CGRect region = CGRectMake(ceil(startX - 3), ceil(lineOrigin.y - _fontLineHeight + _fontLineHeight * 0.1f), ceil(endX - startX + 6), ceil(_fontLineSpacing));
                if (block)
                    block(region);
            }
        }
...
+ (TGReusableLabelLayoutData *)calculateLayout:(NSString *)text additionalAttributes:(NSArray *)additionalAttributes textCheckingResults:(NSArray *)textCheckingResults font:(CTFontRef)font textColor:(UIColor *)textColor frame:(CGRect)frame orMaxWidth:(float)maxWidth flags:(int)flags textAlignment:(NSTextAlignment)textAlignment outIsRTL:(bool *)outIsRTL additionalTrailingWidth:(CGFloat)additionalTrailingWidth maxNumberOfLines:(NSUInteger)maxNumberOfLines numberOfLinesToInset:(NSUInteger)numberOfLinesToInset linesInset:(CGFloat)linesInset containsEmptyNewline:(bool *)containsEmptyNewline additionalLineSpacing:(CGFloat)additionalLineSpacing ellipsisString:(NSString *)ellipsisString
{
    ...
    CGFloat fontLineHeight = floor(fontAscent) + floor(fontDescent);
    CGFloat fontLineSpacing = floor(fontLineHeight * 1.12f + additionalLineSpacing);
    ...
    if (defaultLinkColor == nil)
      defaultLinkColor = UIColor.blueColor.CGColor;
...
//                     ([match isKindOfClass:[TGTextCheckingResult class]])
            {
                NSString *url = nil;
...
//                }
    ...
    if (flags & TGReusableLabelLayoutMultiline)
    {
        ...
        layout.size = CGSizeMake(floor(rect.size.width), floor(rect.size.height + fontLineHeight * 0.1f));
        ...
        for (int iLine = 0; iLine < numberOfLines; iLine++)
        {
            ...
            for (std::vector<TGLinkData>::iterator it = linksBegin; it != linksEnd; it++)
            {
                ...
                if (intersectionRange.length != 0)
                {
                    ...
                    CGRect region = CGRectMake(ceil(startX - 3), ceil(lineOrigin.y - fontLineHeight + fontLineHeight * 0.1f), ceil(endX - startX + 6), ceil(fontLineSpacing));
...
- (void)drawRect:(CGRect)__unused rect
{
    if (_richText)
      ...
      [TGReusableLabel drawTextInRect:self.bounds text:_text attributedText:_attributedText richText:_richText font:_font highlighted:_highlighted textColor:_textColor highlightedColor:_highlightedTextColor shadowColor:_shadowColor shadowOffset:_shadowOffset numberOfLines:_numberOfLines];
}

+ (void)drawTextInRect:(CGRect)rect text:(NSString *)text attributedText:(NSAttributedString*)attributedText richText:(bool)richText font:(UIFont *)font highlighted:(bool)highlighted textColor:(UIColor *)textColor highlightedColor:(UIColor *)highlightedColor shadowColor:(UIColor *)shadowColor shadowOffset:(CGSize)shadowOffset numberOfLines:(int)numberOfLines
{
...
//#endif
    ...
    for (CFIndex lineIndex = linesRange.location; lineIndex < (CFIndex)(linesRange.location + linesRange.length); lineIndex++)
    {
        ...
        {
            case 1:
                horizontalOffset = floor((rect.size.width - linePosition.lineWidth) / 2.0f);
                break;
            case 2:
